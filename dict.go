...
type AC struct {
	...
	Output  []int64 // hold a word length & type
}
...
func FromDict(dict map[string]int) *AC {
	...
	ac := AC{
		...
		Check:   m.trie.Check,
		Failure: m.failure,
		Output:  make([]int64, len(m.failure)),
	}
	...
	for state, words := range m.output {
		...
		ac.Output[state] = int64(wordType << BitLenType) | (int64(maxLength) & MaskLength)
	}
	return &ac
}
